...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  String generateFakeEmail(String identifier) {
    int timestamp = DateTime.now().millisecondsSinceEpoch; // رقم فريد لكل مستخدم
    return "$identifier$timestamp@mail.com"; // تركيب الإيميل
  }

  /***شااااااااااااااااات***/
  bool isOTPSent = false; // لمعرفة إذا تم إرسال OTP
  bool isOTPVerified = false; // لمعرفة إذا تحقق OTP بنجاح
  TextEditingController otpCont = TextEditingController(); // حقل OTP
/*يييييييييييييييييي*/
  ...
  Future<void> registerWithOTP() async {
    ...
    if (formKey.currentState!.validate()) {
      if (isAcceptedTc) {
        ...
        String phone = widget.phoneNumber.validate().trim();
        if (phone.startsWith('0')) {
          phone = phone.substring(1);
        }

        String countryCode = selectedCountry.phoneCode; // مثلاً "962"
        UserData userResponse = UserData()
          ...
          ..email = "${buildMobileNumber().replaceAll('+', '').replaceAll('-', '')}@mail.com" // <-- هنا
          ..firstName = fNameCont.text.trim()
          ..lastName = lNameCont.text.trim()
          ..userType = USER_TYPE_USER
          ..uid = widget.uid.validate()
          ..password = passwordCont.text.trim(); // فقط ما أدخله المستخدم
  ...
  void registerUser() async {
    ...
    if (formKey.currentState!.validate()) {
      ...
      if (isAcceptedTc) {
        ...
        String phone = mobileCont.text.trim();
        if (phone.startsWith('0')) {
          phone = phone.substring(1);
        }
  ...
  Widget _buildFormWidget() {
    ...
    return Column(
      children: [
        ...
        16.height,
        Offstage(
          offstage: true, //  يخفي الحقل عن المستخدم
          child: AppTextField(
...
//onTap(() => changeCountry()),
            ...
            Offstage(
              offstage: true, // true يعني مخفي تماماً
              child: AppTextField(
